...
export default function Home() {
  ...
  async function fetchComments(videoId) {
    const response = await fetch(
      `https://www.googleapis.com/youtube/v3/commentThreads?videoId=${videoId}&key=AIzaSyCpzG8jvXsTqPQIa-HBo4TmYxI71COX4BU&part=snippet,replies&maxResults=100`
    );
  ...
  return (
    <main className={`flex min-h-screen flex-col items-center`}>
      ...
      <div className="border-8 flex items-strech w-1/2 mt-10 bg-green-200 border-green-200 dark:border-green-700 dark:bg-green-700 text-black text-xl p-2 h-min sm:w-11/12">
        👽 <span className="ml-1 dark:text-white">This Analyzer is not 100% accurate</span>
      </div>
      ...
      <div className="rounded-full h-10 overflow-hidden min-w-min">
